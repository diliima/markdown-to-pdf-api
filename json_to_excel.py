...
class JsonToExcel:
    ...
    Suporta formatação customizada e múltiplas abas.
    """
    ...
    def json_to_excel_multiple_sheets(self, data_dict: Dict[str, List[Dict[str, Any]]], 
                                    output_path: str, apply_formatting: bool = True) -> bool:
        """
        Converte múltiplos conjuntos de dados JSON para um arquivo Excel com múltiplas abas.
        
        Args:
            data_dict: Dicionário onde chave=nome_da_aba, valor=dados_json
            output_path: Caminho onde salvar o arquivo Excel
            apply_formatting: Se deve aplicar formatação
        ...
        """
        try:
            logger.info("Iniciando conversão de JSON para Excel (múltiplas abas)...")
            
            if not data_dict:
                logger.error("Dicionário de dados não pode estar vazio")
                return False
            ...
            # Processa cada aba
            for sheet_name, json_data in data_dict.items():
                if not json_data or not isinstance(json_data, list):
                    logger.warning(f"Dados para aba '{sheet_name}' são inválidos, pulando...")
                    continue
                ...
                logger.info(f"Aba '{sheet_name}': {len(df)} linhas, {len(df.columns)} colunas")
            
            # Verifica se pelo menos uma aba foi criada
            if not self.workbook.worksheets:
                logger.error("Nenhuma aba foi criada com dados válidos")
                return False
            
            # Salva o arquivo
            self.workbook.save(output_path)
            
            logger.info(f"Planilha Excel com múltiplas abas gerada: {output_path}")
            
            return True
            
        except Exception as e:
            logger.error(f"Erro ao converter JSON para Excel (múltiplas abas): {str(e)}")
            return False
    
    def validate_json_data(self, json_data: Any) -> tuple[bool, str]:
        """
            ...
            if json_data:
                ...
                for i, item in enumerate(json_data[1:], 1):
                    if set(item.keys()) != first_keys:
                        return False, f"Item {i} tem chaves diferentes do primeiro item"
